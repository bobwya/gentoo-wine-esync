	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/configure") {
			...
			if (diff_array["ihunk"] == 2) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ "popen \\\\$")) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/include/wine/server_protocol.h") {
			...
			if (diff_array["ihunk"] == 4) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ "^ #define SERVER_PROTOCOL_VERSION [[:digit:]][[:digit:]][[:digit:]]$")) {
					delete file_array[line]
					++diff_array["idiff"]
					++complete
				}
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/server/esync.c") {
			...
			if ((diff_array["ihunk"] == 1) && (complete >= 1)) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^+@END$"))) {
					line_text = " "
					insert_array_entry(file_array, ++line, line_text)
					insert_array_entry(file_array, ++line, line_text)
					++diff_array["idiff"]
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/include/wine/server_protocol.h") {
			...
			if (diff_array["ihunk"] == 4) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^*#define SERVER_PROTOCOL_VERSION "))) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/include/wine/server_protocol.h") {
			...
			if (diff_array["ihunk"] == 4) {
				...
				else if ((diff_array["idiff"] == 2) && sub(text2regexp("struct set_job_completion_port_reply set_job_completion_port_reply;$"), "struct suspend_process_reply suspend_process_reply;", file_array[line])) {
					++diff_array["idiff"]
				}
				else if ((diff_array["idiff"] == 3) && sub(text2regexp("struct terminate_job_reply terminate_job_reply;$"), "struct resume_process_reply resume_process_reply;", file_array[line])) {
					++diff_array["idiff"]
					++complete
				}
				diff_array["exit end line"]=line
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/server/process.c") {
			...
			if (diff_array["ihunk"] == 5) {
				...
				else if ((diff_array["idiff"] == 1) && (file_array[line] ~ text2regexp("^ set_fd_events( process->msg_fd, POLLIN );  /* start listening to events */$"))) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/server/event.c") {
			...
			if (diff_array["ihunk"] == 1) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^ struct object  obj; /* object header */$"))) {
					line_text = (indent "struct list    kernel_object;   /* list of kernel object pointers */")
					insert_array_entry(file_array, ++line, line_text)
					++diff_array["idiff"]
				}
				else if ((diff_array["idiff"] == 3) && (file_array[line] ~ text2regexp("^ static int event_signal( struct object *obj, unsigned int access);$"))) {
				...
				if (diff_array["idiff"] == 1) {
					...
					if ((esync_rebase_index >= 13) || (staging && (esync_rebase_index == 12))) {
						line_text = (indent "event_get_kernel_obj_list, /* get_kernel_obj_list */")
						file_array[line] = line_text
					}
					else if (esync_rebase_index == 12) {
						line_text = (indent "no_kernel_obj_list,        /* get_kernel_obj_list */")
						file_array[line] = line_text
					}
					++diff_array["idiff"]
					++complete
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^ /* initialize it if it didn't already exist */$"))) {
				...
				if (diff_array["idiff"] == 1) {
					...
					else if (sub(text2regexp("struct keyed_event *create_keyed_event( struct object *root, const struct unicode_str *name,$"), "static struct list *event_get_kernel_obj_list( struct object *obj )", file_array[line])) {
						++diff_array["idiff"]
					}
				}
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^ unsigned int attr, const struct security_descriptor *sd )$"))) {
					delete file_array[line]
					indent="     "
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/dlls/ntdll/ntdll.spec") {
			...
			if ((diff_array["ihunk"] == 2) && (complete >= 1)) {
				...
				else if ((diff_array["idiff"] == 3) && sub(text2regexp("pthread_attr_setstack( &attr, teb->DeallocationStack,$"), "pthread_attr_setstack( \\&pthread_attr, teb->DeallocationStack,", file_array[line])) {
					++diff_array["idiff"]
					++complete
				}
			}
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/dlls/ntdll/esync.c") {
			...
			if ((diff_array["ihunk"] == 1) && (complete >= 3)) {
				...
				if (diff_array["idiff"] == 1) {
					if (esync_rebase_index == 7) {
						if (sub(text2regexp("(dump_func)dump_get_new_process_info_request,"), "(dump_func)dump_exec_process_request,", file_array[line])) {
							++diff_array["idiff"]
						}
					}
					else {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/server/process.c") {
			...
			if (diff_array["ihunk"] == 1) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^ static unsigned int process_map_access( struct object *obj, unsigned int access );$"))) {
			...
			if (diff_array["ihunk"] == 1) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^+};$"))) {
					line_text = " "
					insert_array_entry(file_array, ++line, line_text)
					insert_array_entry(file_array, ++line, line_text)
					++diff_array["idiff"]
			...
			if (diff_array["ihunk"] == 2) {
				...
				if (diff_array["idiff"] == 1) {
					if (esync_rebase_index <= 11) {
						...
						split("8 1 8 1", array_diff_lines)
						change_array_entry_diff(file_array, line, array_diff_lines)
						++diff_array["idiff"]
					}
				}
				else if ((diff_array["idiff"] == 2) && sub(text2regexp("struct object obj;"), "& ", file_array[line])) {
					++diff_array["idiff"]
				}
				else if ((diff_array["idiff"] == 3) && sub(text2regexp("struct list devices;"), "& ", file_array[line])) {
					++diff_array["idiff"]
				}
				else if ((diff_array["idiff"] == 4) && sub(text2regexp("struct list requests;"), "& ", file_array[line])) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("list_init( &manager->requests );$"))) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("list_init( &irp->mgr_entry );$"))) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/server/device.c") {
			...
			if (diff_array["ihunk"] == 6) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^ delete_device( device );$"))) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/server/main.c") {
			...
			if (diff_array["ihunk"] == 2) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^ init_signals();$"))) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/dlls/ntdll/thread.c") {
			...
			if (diff_array["ihunk"] == 1) {
				...
				else if ((diff_array["idiff"] == 1) && (file_array[line] ~ text2regexp("^ WINE_DEFAULT_DEBUG_CHANNEL(thread);$"))) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/dlls/ntdll/thread.c") {
			...
			if (diff_array["ihunk"] == 2) {
				...
				else if ((diff_array["idiff"] == 2) && (file_array[line] ~ text2regexp("^ fill_cpu_info();$"))) {
	...
	for (line = 1 ; line <= file_array[0] ; ++line) {
		...
		if (diff_array["file"] == "/dlls/ntdll/ntdll_misc.h") {
			...
			if ((diff_array["ihunk"] == 2) && (complete >= 1)) {
				...
				else if ((diff_array["idiff"] == 2) && (esync_rebase_index <= 4)) {
					diff_array["idiff"] += 2
					++complete
				}
				else if ((diff_array["idiff"] == 2) && sub(text2regexp("pthread_attr_init( &attr );$"), "pthread_attr_init( \\&pthread_attr );", file_array[line])) {
...
function process_patch_file(file_array, diff_array)
{
	...
	if (patch_number == "0001") {
		...
		if (!diff_array["exit code"] && staging) process_staging_patch_file_0025(file_array, diff_array)
	}
		...
		if (!diff_array["exit code"] && staging) process_staging_patch_file_0074(file_array, diff_array)
	}
...
BEGIN{
	supported_patches="0001 0002 0006 0007 0009 0010 0011 0013 0014 0015 0017 0020 0023 0024 0025 0026 0032 0033 0040 0041 0042 0044 0045 0048 0064 0074 0077 0078 0079"
